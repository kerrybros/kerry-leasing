/**
 * Test Idle Events endpoint with different parameter formats
 */
...
async function testIdleEvents() {
  console.log('\n=== Testing Idle Events Endpoint ===\n');
  ...
  for (const format of formats) {
    ...
    try {
      const response = await makeMotiveRequest('/v1/idle_events', format.params);
      ...
      if (records.length > 0) {
        console.log('\n✓✓✓ THIS FORMAT WORKS! ✓✓✓');
        console.log(`\nFirst record sample:`, JSON.stringify(records[0], null, 2).substring(0, 500));
