 ...
 * 
 * Usage:
 *   node --loader tsx src/scripts/setup-wolverine.mjs
 */
...
async function setupWolverineMotive() {
  ...
  const clerkOrgId = 'org_2slAi3SqvSCzvqCJE3i2YtWQCsO'; // Wolverine
  ...
  try {
    ...
    console.log(`  API Key: ${config.credentialsJson.apiKey.substring(0, 10)}...`);
    console.log(`  Status: ${config.status}`);
    console.log(`\n✅ Setup complete!`);
    console.log(`\n📝 Next steps:`);
    console.log(`  1. Run test: node src/scripts/test-yesterday.mjs`);
