...
export async function syncSamsaraOrgForDate(
  ...
  try {
    ...
    if (Date.now() - lastRosterSync > VEHICLE_ROSTER_INTERVAL_MS) {
      const rosterResult = await syncSamsaraVehicles(clerkOrgId, apiToken);
    ...
    const statsResult = await syncSamsaraVehicleStats(clerkOrgId, apiToken);
    ...
    // 4. Safety events
    const safetyResult = await syncSamsaraSafetyEvents(clerkOrgId, apiToken, date);
    ...
    orgResult.duration = Date.now() - startTime;
    console.error(`  ❌ Failed sync for ${clerkOrgId}:`, error.message);
    return orgResult;
  }
}
